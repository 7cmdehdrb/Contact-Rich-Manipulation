...
| sweep command, phase 전환 | `mdp/commands.py` |
...
시작점/목표점 시각화 구현과 event는 현재 모두 주석 처리되어 있고 command의
`debug_vis=False`가 적용된다. 대신 Reset 마지막에 테스트용
`print_reset_physics_info` event가 실행되어 다음 정보를 출력한다.

- TargetCube 질량
- TargetCube collision shape의 `(static friction, dynamic friction)` 고유값
- Shelf collision shape의 `(static friction, dynamic friction)` 고유값

한 번에 최대 8개 환경만 출력하고 나머지 개수는 별도로 표시한다. 확인이 끝나면
`EventsCfg`의 `print_reset_physics_info = EventTerm(...)` 블록만 주석 처리하면 된다.
이 함수는 PhysX 값을 읽기만 하며 simulation state를 변경하지 않는다.
