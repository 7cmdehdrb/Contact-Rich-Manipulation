...
namespace inspire_hand
{
    hand_serial::hand_serial(ros::NodeHandle *nh) : act_position_(-1),hand_state_(0xff)
    {
        ...
        // com_port_glove = new serial::Serial(port_name_glove, (uint32_t)baudrate_, serial::Timeout::simpleTimeout(100));
        if (com_port_->isOpen())// && com_port_glove->isOpen()
        {
            ROS_INFO_STREAM("Hand: Serial port " << port_name_ << " openned");// << port_name_glove << " openned"
            int id_state = 0;
            while (1)
            {
                // start_glove(com_port_glove);
    ...
    // hand_serial::start_glove(serial::Serial *port)
    // {
    //     ros::Duration(0.015).sleep();
