...
WideRandomization, TactileLocalization, ConstantVelocity, Gripper Exclusion,
HomeReturn을 포함한 전체 7개 환경의 차이는
[등록 환경 문서](registered_osc_sweep_environments.md)를 기준으로 한다.
...
Windows에서는 실행 예시의 `./IsaacLab/isaaclab.sh`를
`.\IsaacLab\isaaclab.bat`로 바꿔 사용한다. 문서 내용은 2026-07-19 현재 코드 기준이다.
