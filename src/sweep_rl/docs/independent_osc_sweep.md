...
56-D 관측, 12-D OSC 액션, 네 reward의 계산식과 Domain Randomization lifecycle을
구현 단위로 확인하려면
[패키지 환경 명세](../sweep_rl/osc_sweep_independent/README.md)를 함께 참고한다.

## 1. 환경 개요

이 환경은 UR5e와 항상 열린 Robotiq 2F-85 gripper를 이용하여 정육면체 물체를
지정된 방향, 거리, 속도로 민 뒤, 물체를 다시 건드리지 않고 로봇을 Home joint
pose로 복귀시키는 3단계 강화학습 환경이다.
...
```text
REACH (task_phase=0)
  1. 물체의 push 반대편 옆면으로 접근
  2. gripper 외측 pad로 접촉
       ↓ filtered target contact > 0.25 N
SWEEP (task_phase=1)
...
REACH, SWEEP, HOME 모두 동일한 12-D OSC policy가 제어한다. HOME phase에서 별도의
...
작업 공간은 `example/Sweep-Policy/sweeping_policy/shelf_sweep_env_cfg.py`의
`speedrack_shape.usd` 선반을 사용한다. 로봇 베이스 pose와 Home joint pose도 해당
예제의 UR5e 설정을 따르되, virtual F/T link와 좌·우 contact pad 조립 구조는 유지한다.
...
Physics replication은 물체 크기를 병렬 환경마다 다르게 적용하고 robot rigid body별
filtered contact sensor를 구성하기 위해 비활성화한다.
...
| 9 | Task Phase | 1 | REACH=`0`, SWEEP=`1`, HOME=`2` | 없음 |
...
Target pad contact가 생기면 phase가 REACH에서 SWEEP으로 전환되고 Reaching reward는
0이 되며 Contact/Push 목적이 중심이 된다.
...
`0`을 반환한다. Shelf나 다른 robot link와의 접촉은 이 reward로 인정하지 않는다.
...
| Invalid target pose | target center 높이 < 1.04 m 또는 roll/pitch 절댓값 > 0.80 rad |
| Excessive F/T | force norm > 100 N 또는 torque norm > 15 Nm |
| Arm speed | arm joint 중 하나라도 절대 속도 > 6.5 rad/s |
| Shelf collision | UR5e arm, Robotiq gripper 또는 sweep contact pad와 shelf 사이의 filtered contact force > 0.1 N |
| Robot self-collision | 직접 연결/중립 자세에서 겹치는 예외 쌍을 제외한 UR5e·Robotiq·pad rigid body 사이 filtered contact force > 0.1 N |

Timeout은 Isaac Lab의 time-out termination으로 기록되고 나머지는 실패 termination으로
기록된다. 실패 termination에 연결된 별도 reward penalty는 없다.

PhysX filtered ContactSensor는 환경당 하나의 sensor body만 여러 filter body와 비교할 수
있다. 따라서 UR5e 7개 body, Robotiq 9개 body, 좌·우 contact pad 2개에 센서를 각각 두고
모든 센서가 `TargetCube`, shelf 내부 rigid body `Shelf/rack`, 그리고 18개 robot rigid
body를 명시적으로 filter하도록 구성한다.
이 구조로 다중 환경에서 Robot wildcard가 모든 link를 한꺼번에 매칭하던 filter-count
오류를 방지한다.

Self-collision physics는 활성화되어 있다. 다만 fixed/revolute joint로 직접 연결되거나
shelf 예제의 중립 gripper pose에서 의도적으로 맞닿는 다음 다섯 쌍은 termination에서
제외한다: wrist_3–gripper base, gripper base–좌/우 inner knuckle, 좌 inner
finger–right pad, 우 inner finger–left pad. 그 밖의 비인접 UR 링크끼리, gripper
링크끼리, UR과 gripper 사이의 충돌은 모두 실패 종료 대상이다.
...
기본 물체 중심은 robot base 기준 대략 `(0.60, 0.0)`이다. Z 위치는 고정 offset으로
randomize하지 않고 실제 sampled cube size의 절반을 shelf surface 높이 `1.05 m`에 더해
항상 물체 바닥면이 shelf 위에 놓이도록 계산한다. Reset 시 linear/angular velocity는
0으로 초기화한다.
...
### 7.2 Object–Shelf Friction

Target과 shelf material을 simulator startup 시 병렬 환경별로 독립 배정한다. 각각 64개
material bucket을 사용하며 dynamic friction이 static friction을 넘지 않게 제한한다.

| Asset | Static friction | Dynamic friction | Restitution |
|---|---:|---:|---:|
| TargetCube | 0.40–1.10 | 0.25–0.90 | 0.0 |
| Shelf | 0.40–1.10 | 0.25–0.90 | 0.0 |

한 simulator 실행에서 다양한 object–shelf 조합을 병렬로 경험하게 하여 실제 선반의
표면 재질이나 마모 상태가 시뮬레이션과 조금 달라도 대응할 수 있도록 한다.
...
방향과 거리를 단순히 독립 sampling하면 긴 command가 shelf 또는 robot 작업영역 밖에
...
```text
Robot-root X workspace:  0.50–0.90 m
Robot-root Y workspace: -0.50–0.50 m
추가 boundary margin: 0.015 m + cube half-size
```

이는 composed shelf USD 중간 상판의 world bounds X `[-0.90,-0.50]`,
Y `[-0.50,0.50]`를 기준으로 한다. 로봇 root quaternion `(0,0,0,1)`이 Z축 180도
회전이므로 robot-root command frame에서는 X 부호가 반전된다.
...
### 7.4 OSC Parameter Randomization
...
```powershell
./IsaacLab/isaaclab.sh -p src/sweep_rl/scripts/train_independent_sweep.py --num_envs 4096 --device cuda:0 --headless
```
