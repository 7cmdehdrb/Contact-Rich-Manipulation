...
기존 환경의 일정 속도 sweep, 목표점 정지, upright gripper, `0.06~0.12 m` cube
크기 랜덤화는 유지하고, 물체를 놓은 뒤 UR5e가 Home joint pose로 복귀하는 두 번째
단계를 추가한다.
...
| 값 | Phase | 동작 |
|---:|---|---|
| 0 | `SWEEP` | 기존 UprightRandomSize reward로 물체를 목표점까지 이동 |
...
| Observation | 차원 |
|---|---:|
| 부모 UprightRandomSize Observation | 55 |
| `task_phase` | 1 |
| **합계** | **56** |
...
기존 UprightRandomSize 정책으로부터 이어서 학습하려면 observation이 55-D에서
