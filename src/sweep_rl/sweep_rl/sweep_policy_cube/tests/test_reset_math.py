"""Tests for the direction-aware Cube reset sampler."""
...
def test_each_environment_uses_its_selected_slot_and_jitter():
    ...
    result = reset_math.sample_slot_positions(indices, slots, jitter, 1.125)
    expected = torch.tensor(
        [[-0.73, -0.22, 1.125], [-0.75, 0.01, 1.125], [-0.61, 0.00, 1.125]]
    )
    torch.testing.assert_close(result, expected)


def test_center_height_overrides_slot_z_without_mutating_slots():
    slots = torch.tensor([[-0.60, 0.00, 99.0]])
    original = slots.clone()
    result = reset_math.sample_slot_positions(
        torch.tensor([0]), slots, torch.zeros(1, 2), 1.125
    )
    torch.testing.assert_close(result[0, 2], torch.tensor(1.125))
