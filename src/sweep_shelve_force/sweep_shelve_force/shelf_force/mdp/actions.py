...
class ShelfOperationalSpaceAction(OperationalSpaceControllerAction):
    """Map ``[stiffness(6), relative_pose_rpy(6)]`` to OSC joint efforts."""

    cfg: "ShelfOperationalSpaceActionCfg"

    def __init__(self, cfg: "ShelfOperationalSpaceActionCfg", env: ManagerBasedEnv):
        super().__init__(cfg, env)
        if self.action_dim != 12:
            raise ValueError(f"Shelf OSC must be 12-D, received {self.action_dim}.")
    ...
    def _preprocess_actions(self, actions: torch.Tensor):
        ...
        if actions.shape != expected_shape:
            raise ValueError(
                f"Expected shelf OSC action shape {expected_shape}, got {tuple(actions.shape)}."
            )
        ...
        self._torque_saturated[:] = (~finite.all(dim=1)) | torch.any(
...
@configclass
class ShelfOperationalSpaceActionCfg(OperationalSpaceControllerActionCfg):
    class_type: type = ShelfOperationalSpaceAction
